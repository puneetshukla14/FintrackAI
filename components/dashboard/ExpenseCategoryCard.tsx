...
{/* Chart Section - Premium Glassmorphic Donut */}
<motion.div
...
>
  <div className="relative w-full max-w-[340px] aspect-square p-6 rounded-[2rem] bg-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.15)] backdrop-blur-lg border border-white/10">
    <Doughnut
      data={{
        labels: rows.map(([name]) => name),
        datasets: [{
        ...
        ₹{totalSpent.toLocaleString('en-IN')}
          ...
          >
            {rows.map(([cat, amt]) => (
              <li
                key={cat}
                className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all backdrop-blur"
              >
                <div className="flex items-center gap-3">
                  <span className="text-xl">{CATEGORY_ICONS[cat] || <FiBox />}</span>
                  <span className="text-base font-medium">{cat}</span>
                </div>
                <span className="text-emerald-400 font-semibold tracking-tight">₹{amt.toFixed(2)}</span>
              </li>
            ))}
          </motion.ul>
        </div>
